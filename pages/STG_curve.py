...
st.title("Create Steam Turbine Performance Curve with less manual work")
...
def cal_n():
    A = np.array([[opr_x**2, opr_x, 1], [X_max**2, X_max, 1], [0.0**2, 0.0, 1]])
    B = np.array([opr_y, Y_max, 0.0])
    try:
...
Y_points = n[0] * X_points**2 + n[1] * X_points + n[0]
...
st.download_button(
    label = "CSV", 
    data = csv_file,
    file_name = "STG_curve.csv", 
    mime = "text/csv"
)

fig = go.Figure()
fig.add_trace(go.Scatter(x = X_points, y  = Y_points, mode ='lines+markers', name='STG Curves'))
...
fig.update_layout(
    title=dict(text= "STG Curve", font=dict(color='white') ),
